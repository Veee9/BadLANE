...
class LaneATT(nn.Module):
    def __init__(self,
        ...
        # Some definitions
        self.feature_extractor, backbone_nb_channels, self.stride = get_backbone(backbone, pretrained_backbone, pretrained_path)
        self.img_w = img_w
        self.n_strips = S - 1
        self.n_offsets = S
    ...
    def loss_pgd(self, proposals_list, targets, cls_loss_weight=10):
        focal_loss = FocalLoss(alpha=0.25, gamma=2.)
        smooth_l1_loss = nn.SmoothL1Loss()
        cls_loss = 0
        reg_loss = 0
...
def get_backbone(backbone, pretrained=False, pretrained_path='none'):
    if backbone == 'resnet122':
        ...
        if pretrained_path == 'none':
            ...
            print("load my pretrained model")
            backbone = torch.nn.Sequential(*list(resnet34(pretrained=False).children())[:-2])
