...
class Trainer(object):
    ...
    def __init__(self, flow, adv_models, query_set_models, optim, train_set, valid_set, args, cuda):
        ...
        self.model_name = args.support_set
    ...
    def adv_loss(self, y, label):
        ...
        for adv_model in self.adv_models:
            logits = adv_model(y)
            true_logits = adv_model(label)
            margin = self.criterion(logits, true_logits)
    ...
    def meta_train(self, batch_data, epoch):
        ...
        _flow = copy.deepcopy(self.flow.state_dict())
    ...
    def meta_test(self):
        ...
        with torch.no_grad():
            for batch_data in self.validset_loader:
                # images = batch_data[0]
                # labels = batch_data[1]
                images = batch_data['cln_img']
                labels = batch_data['pois_img']
                if self.cuda:
                    images = images.cuda()
                    labels = labels.cuda()
                labels = labels.long()
